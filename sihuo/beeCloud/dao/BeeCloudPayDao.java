...
public class BeeCloudPayDao {
	...
	// 根据商户号查询拿到一个实体
	public MercantEnterInfo queryMerchantBysaruId(String saruLruid) {
		String sql = "select SARU_CHIEF,SARU_CERTNO,SARU_PHONE,SARU_BACKNAME,SARU_BANKLINKED ,SARU_BACKCARD from r_b_saleru where saru_lruid=?";
		Connection con = null;
		PreparedStatement state = null;
		ResultSet set = null;
		MercantEnterInfo consumer = new MercantEnterInfo();
		try {
			con = ConnectionSource.getConnection();
			state = con.prepareStatement(sql);
			state.setString(1, saruLruid);
